...
const tileServers = {
  ...
  'Esri Satellite': { url: 'https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}', attribution: 'Tiles &copy; Esri &mdash; Source: Esri, i-cubed, USDA, USGS, AEX, GeoEye, Getmapping, Aerogrid, IGN, IGP, UPR-EGP, and the GIS User Community', subdomains: 'abcd' },
};
...
export default class WorldMap {
  ...
  createLegend() {
      /*Aqui se cambio la posicion del control hacia la derecha y abajo*/ 
    this.legend = (<any>window).L.control({ position: 'bottomright' });
    this.legend.onAdd = () => {
    ...
    this.legend.addTo(this.map);
     ...
     /*Aqui se crea el div con el logo de Softtek*/ 
     this.logo = (<any>window).L.control({ position: 'bottomleft' });
     this.logo.onAdd = () => {
       this.logo._div = (<any>window).L.DomUtil.create('div', 'logo');
       this.logo.update();
       return this.logo._div;
     };
     this.logo.update = () => {
       /*logo de cinepolis*/
      /* let logo = '<img width="75px" src="https://static.cinepolis.com/img/lg-cinepolis-new.png"></img>'
       */
       /*logo softtek*/ 
       let logo = '<img width="75px" src="https://www.softtek.com/images/content/design2015/LogoCompleto-Website-20.png"></img>'
       
       this.logo._div.innerHTML = logo;
     }
  ...
  removeLegend() {
    ...
    this.legend = null;
    /*Para remover el Logo*/
    this.logo.remove(this.map);
    this.logo = null;
  }
